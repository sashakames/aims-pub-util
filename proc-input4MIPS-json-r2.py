...
#	DRS = parts[1:6] + parts[7:10] # skip mip era and realm
	...
	DRS = parts[0:3] + [parts[7]] + [parts[5]] + parts[8:10]
	 
	fn = parts[-1]
	version = sys.argv[3]

	pathlst = [path_prefix] + DRS + [version  ,fn]
	openpath = '/'.join(pathlst)
	ncfd = cdms2.open(openpath)
	...
	for baseattr in attr_key_lst:
		...
		if len(value) < 1:
			...
			try:
				value = ncfd.__getattribute__(attr)
			#	write missing attributes back to json
			except:
				value =""
			obj[attr] = value
